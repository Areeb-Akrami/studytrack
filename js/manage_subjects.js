    ...
    subjects.forEach((subject, index) => {
        ...
        div.innerHTML = `
            <div class="subject-info">
                <h3>${subject.name}</h3>
                <p>${subject.code ? `Code: ${subject.code}` : ''} ${subject.code && subject.totalClasses ? '| ' : ''}${subject.totalClasses ? `Classes per week: ${subject.totalClasses}` : ''}</p>
            </div>
...
document.getElementById('add-subject-form').addEventListener('submit', function(e) {
    ...
    const subject = {
        ...
        totalClasses: document.getElementById('total-classes').value ? parseInt(document.getElementById('total-classes').value) : null,
